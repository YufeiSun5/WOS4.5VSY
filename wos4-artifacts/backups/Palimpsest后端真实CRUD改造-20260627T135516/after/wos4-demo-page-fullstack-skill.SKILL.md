...
- 前端按钮先只调用 `QueryAssessmentRecords` 做 smoke，确认 `ret=0` 和后端返回 JSON 能显示到按钮或页面。
- 后端模型层 `CreateAssessmentRecord / UpdateAssessmentRecord / DeleteAssessmentRecord / SubmitAssessmentScore` 已在 2026-06-27 改成真实 `Create/Update/Delete` 业务事记录 API，并通过模型编辑器编译提交。
- 这些 CRUD 函数的运行态版本仍依赖后续运行包更新、部署/启动、重新打开客户端。页面预览没有复测到新运行包前，前端按钮只能标为“待联调真实 CRUD”，不能写成已完成。
- 页面按钮调用 Update/Delete/Submit 时必须从表格行里带 `recordID`。没有 `recordID` 的假数据行只能走前端演示或返回 `VALIDATION_FAILED`，不要把学号当作记录 ID。
...
7. 后端联调：先 `Backend Smoke`，再替换真实 Query；真实 CRUD 必须记录 `ret/errorcodes/output` 和 `recordID` 来源，失败也返回给前端。
