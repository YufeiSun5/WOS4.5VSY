...
- Keep Create/Update/Delete honest: until their `Create/Update/Delete(param,input,output)` record parameter shape is verified from the manual or a captured working request, those functions must return structured error JSON or call a verified helper. Do not silently fall back to fake success.
