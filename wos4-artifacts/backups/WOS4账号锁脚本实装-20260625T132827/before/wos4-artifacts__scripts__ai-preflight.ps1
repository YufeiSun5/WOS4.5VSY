...
if ([string]::IsNullOrWhiteSpace($ExpectedDeveloper) -and -not [string]::IsNullOrWhiteSpace($env:WOS4_EXPECTED_DEVELOPER)) {
  $ExpectedDeveloper = $env:WOS4_EXPECTED_DEVELOPER
}
if ([string]::IsNullOrWhiteSpace($ExpectedUrl) -and -not [string]::IsNullOrWhiteSpace($env:WOS4_EXPECTED_URL)) {
  $ExpectedUrl = $env:WOS4_EXPECTED_URL
}
...
if (Test-Path -LiteralPath $iniPath) {
  ...
  if ([string]::IsNullOrWhiteSpace($developerName)) {
    ...
    $failed = $true
  } elseif ([string]::IsNullOrWhiteSpace($ExpectedDeveloper)) {
    Add-Result $results "ini.identity" "pass" "developer_name is present. No expected developer was supplied."
  } elseif ($developerName -eq $ExpectedDeveloper) {
  ...
  if ($missingWos4.Count -gt 0) {
    Add-Result $results "ini.wos4" "fail" ("Missing wos4 fields: " + ($missingWos4 -join ", "))
    $failed = $true
  } elseif (-not [string]::IsNullOrWhiteSpace($ExpectedUrl) -and $wos4Url -ne $ExpectedUrl) {
    Add-Result $results "ini.wos4" "fail" "wos4 url does not match expected URL."
    $failed = $true
  } else {
...
$summary = [ordered]@{
  ...
  expectedDeveloperEnforced = -not [string]::IsNullOrWhiteSpace($ExpectedDeveloper)
  expectedUrlEnforced = -not [string]::IsNullOrWhiteSpace($ExpectedUrl)
  taskPath = $TaskPath
  wos4Mode = [bool]$Wos4
