...
新交互项必须记录“开发人员 + AI 身份”，不要只写裸 AI 身份。
...
- 用户明确要求 close 时，当前执行 AI 必须在同一次修改里完成归档、移出面板和更新归档索引，并在归档中写明 `关闭触发：用户`。
- AI 自行判断 close 时，只有“发起人 AI”可以关闭事项。
- 参与人可以反馈 `approved`、`needs-change` 或 `blocked`，但不能直接关闭。
- close 是原子操作：选择 close 后必须立刻把完整事项归档到 `.ai/closed-interactions/<日期>-<编号>-<简短标题>.md`。
- 归档文件必须写明 `状态：closed`、关闭人、关闭时间、关闭原因和最终结论。
- 归档完成后，必须从本文件“当前事项”中移出该事项。
- 每次归档后必须同步更新 `.ai/closed-interactions/INDEX.md`；查询历史事项时先读 `.ai/closed-interactions/AI-ROUTE.md` 和 `INDEX.md`，再打开具体归档文件。
- 不允许只把状态改成 `closed` 后留在本文件里，也不允许等文件变长后再集中归档。
...
- `wos4-artifacts/tasks/20260623-验证蓝色客户端创建skill/`
...
验证“蓝色客户端对象创建”是否已经从一次性探索沉淀为可复用 skill。详细检查项见任务工作包 `01-需求说明.md`。
...
- 待 `孙宇飞_test-ai` 和 `孙宇飞_review-ai` 按任务工作包执行验证。

关闭条件：

- 测试和审阅结论均写入任务工作包。
- 发起人 AI `孙宇飞_code-ai` 确认可关闭。
...
- 当前推荐：优先验证客户端双页面并排；结构固定为左侧 `PalimpsestMenu` + 右侧 `PalimpsestContent`，若正式运行态不稳定，再降级为单页面左右分栏。
