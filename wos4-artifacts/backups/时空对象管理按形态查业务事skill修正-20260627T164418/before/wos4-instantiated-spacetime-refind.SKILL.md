...
2026-06-27 follow-up for `PalimpsestL1_0626R2`: after selecting `时空对象管理平台 -> PalimpsestL1_0626R2 -> 计划 -> 业务事`, the runtime list showed `共 5 条`, not 8. Visible objects were:
...
Missing runtime business events were:
...
In this state, backend CRUD functions targeting `pal_assessment_record` cannot be accepted as real persistence tests. Re-route to model version / instance configuration / operations deployment until the missing objects appear in object management. Do not create these objects manually inside object management.
