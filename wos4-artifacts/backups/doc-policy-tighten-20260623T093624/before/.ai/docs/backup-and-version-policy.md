...
2. 对要改的文件，优先依赖 Git diff 作为版本控制。
3. 涉及批量删除、迁移、覆盖、格式化、大范围重写时，先复制到 `wos4-artifacts/backups/<任务名>-<时间>/before/` 或 `before-delete/`。
4. 备份目录必须包含清单，例如 `cleanup-manifest.json`、`backup-manifest.json` 或 README。
5. 修改后更新 `MEMORY.md`。
6. 如果备份内容包含旧脚本、抓包、截图或其他可能含敏感信息的文件，原始备份目录只本机保留，不进 Git；只提交清单和说明。
