...
1. 运行 `wos4-lock.ps1 -Action AcquireAccount -Owner <当前AI身份> -Task <任务说明>` 申请账号锁。
2. 如果锁脚本返回 `blocked`，停止登录并把锁冲突原因报告给用户。
3. 用锁结果中的 `account/session/isolated_context/profile_dir` 选择浏览器上下文或独立 profile。
4. 如果用户要求前台可见，先尝试 Chrome MCP。
5. 打开登录页。
6. 等待两个输入框出现。
7. 填用户名和密码。
8. 如果普通 fill/type 后密码为空或按钮仍禁用，使用原生 value setter。
9. 点击“登录”。
10. 等 URL 到 `#/main`，或页面出现 `时空功能开发`、`系统运维`、`安全管理`、`审计管理`。
11. 当前任务结束、账号不再使用或用户要求归还时，运行 `wos4-lock.ps1 -Action ReleaseAccount`。
