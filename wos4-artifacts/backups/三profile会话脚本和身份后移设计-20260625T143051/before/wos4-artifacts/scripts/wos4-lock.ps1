param(
  [ValidateSet("Status", "ListAccounts", "AcquireAccount", "ReleaseAccount", "ForceReleaseAccount", "MarkAccountStale", "AcquireObject", "ReleaseObject")]
  ...
  if ($sessions.Count -eq 0) {
    ...
    foreach ($accountName in $AccountOrder) {
      $sessions["slot$i"] = [ordered]@{
        ...
        since = ""; note = ""; driver = ""; profile_dir = ""; isolated_context = ""
      }
      $i++
    }
  }
