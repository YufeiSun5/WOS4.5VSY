...
- preflight 默认不写死开发人员或 URL，只检查 `wos4.local.ini` 字段存在；需要强校验时传 `-ExpectedDeveloper`、`-ExpectedUrl`，或设置 `WOS4_EXPECTED_DEVELOPER`、`WOS4_EXPECTED_URL`。
...
- browser-harness 并发会话必须使用不同 `profile_dir`、`cdp_port` 和 `harness_name`；启动独立 Chrome profile 只表示浏览器资源就绪，不表示账号或 AI 身份已被占用。
...
- WOS4 并发开发中的账号占用和 AI 身份认领以后移到 `AcquireAccount -Owner <开发人员_AI身份>` 成功时为准；不得在 Chrome profile 启动阶段就认定 `某某_test-ai` 或 `某某_code-ai` 已占用账号。
- 当前 AI 不再使用某个 WOS4 账号时，必须运行 `wos4-lock.ps1 -Action ReleaseAccount` 归还账号使用权；异常中断恢复后先查锁状态，不得直接重复登录。
- 不做账号席位定时自动释放；开发中途不能因超时释放账号。需要归还时显式执行 `ReleaseAccount`，或用 `ReleaseOwnerAccounts -Owner <开发人员_AI身份>` 归还当前 AI 身份持有且没有对象锁的账号。
...
- Git 提交记录必须使用中文；必要的技术标识、远端名、分支名、文件名、函数名、平台对象名、错误码可以保留原文。
...
- 每次上下文压缩、线程恢复、暂停后继续、跨天继续或接手未完成任务时，AI 在继续执行前必须先声明本轮采用的身份，例如 `孙宇飞_design-ai`、`孙宇飞_frontend-ai`、`孙宇飞_code-ai`、`孙宇飞_test-ai` 或 `孙宇飞_review-ai`。
- 声明身份后，必须读取对应 `.ai/agents/<身份>.md`，再按任务类型读取匹配的 skill、instructions 和 docs；不能在未确认身份的情况下直接继续旧任务。
- 如果当前任务性质和原身份不一致，例如从方案讨论转为实现、从实现转为审阅，必须明确说明身份切换原因，并按新身份重新读取对应 agent 文件和 skill。
- 恢复后发送给用户的第一条状态更新，应包含“当前身份 + 正在恢复的任务 + 下一步检查项”，避免用户无法判断当前是 design-ai、code-ai、test-ai 还是 review-ai 在操作。
