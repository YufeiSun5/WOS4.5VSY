...
`[wos4]` 放共享入口、账号顺序和锁文件；日常开发账号放在 `[wos4.account.<账号别名>]`。运维、部署、调试访问等账号必须放在独立段里。
...
该脚本检查 Git 脏状态、敏感路径忽略、真实 ini 身份、必需 agent、WOS4 skill 和任务包结构；默认不写死开发人员或 URL，只要求 ini 字段存在。需要强校验时传 `-ExpectedDeveloper`、`-ExpectedUrl`，或设置 `WOS4_EXPECTED_DEVELOPER`、`WOS4_EXPECTED_URL`。失败时先修复，不继续登录、保存、提交、预览或提交 Git。
