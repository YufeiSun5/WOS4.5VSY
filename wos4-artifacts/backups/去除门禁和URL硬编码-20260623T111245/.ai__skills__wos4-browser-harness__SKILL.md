...
cfg = {}
for line in open(r'D:\DEV_D\WOS4.5\wos4-artifacts\config\wos4.local.ini', encoding='utf-8'):
    m = re.match(r'^([^#;\[\]=]+)=(.*)$', line.strip())
    if m:
        cfg[m.group(1).strip()] = m.group(2).strip()

ensure_real_tab()
new_tab(cfg.get('url', 'http://221.239.19.118:13001/#/login'))
