...
- 在 `组态系统客户端0518 / KingFusion4.5_工程浏览器` 中，左侧存在“父菜单只负责展开/收起、子菜单才是真实页面”的结构。不得把父级菜单组当成内容页入口。
...
已验证 `建模系统客户端_0518` 也遵循桌面应用规则：

```text
WOS4 登录页
-> 主页面 #/main
-> 建模系统客户端_0518 卡片图标真实双击
...
- 卡片 DOM 示例：`.data-item.card-item[title="建模系统客户端_0518"]`，实测卡片约 `68x112`，双击图标区比双击文字更稳定。
