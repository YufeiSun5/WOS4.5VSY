param(
  ...
  [int]$WindowLeft = 0,
  [int]$WindowTop = 0,
  [int]$WindowOffsetX = 48,
  [int]$WindowOffsetY = 32,
  [switch]$NoHarness
)
  ...
  do {
    ...
    if ($status.ok) {
      return [ordered]@{
        ...
        action = "started"
        window = [ordered]@{ width = $WindowWidth; height = $WindowHeight; left = $WindowLeft; top = $WindowTop }
        cdp = $status
      }
    }
...
if ($Action -eq "Status") {
  ...
  $sessionIndex = 0
  foreach ($s in $sessions) {
    $left = $WindowLeft + ($sessionIndex * $WindowOffsetX)
    $top = $WindowTop + ($sessionIndex * $WindowOffsetY)
    $startResult = Start-ChromeSession -SessionInfo $s -ChromeExe $chromeExe -StartUrl $StartUrl -WindowWidth $WindowWidth -WindowHeight $WindowHeight -WindowLeft $left -WindowTop $top
