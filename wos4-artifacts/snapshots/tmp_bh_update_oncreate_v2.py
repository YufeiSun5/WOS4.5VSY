...
Trace(2, "onCreate.QueryAssessmentRecords.result", resultText);`;
  const req={param:me({type:de.TYPE_FUNCTION,areaid:0},{type:'int32Value',areaid:'int32Value'}), input:me({names:['onCreate'],modelType:de.TYPE_FUNCTIONUNITMODEL,identifierType:1,modelId:modelId,...St([me({$script:scriptText},io)])},{names:'stringArrayValue',modelType:'int32Value',identifierType:'int32Value',modelId:'uint64Value',version:'int32Value',datafields:'stringArrayValue',datavalues:'valueArray2DValue'})};
  const r=await Ke('Update', req);
  return {updateRet:r&&r.ret, raw:r, scriptLength:scriptText.length};
}
return main();
''')
print(res)
