...
- 后端自定义计算开发和调试第一阶段必须进入 `建模系统客户端` 的后端模型函数编辑器。`时空功能开发平台` 是时空/仓库包/运行侧链路入口，不是源模型自定义计算开发入口；如果用户要求“调后端”“调自定义计算”“在 onCreate 里调试”，不要把 `时空功能开发平台` 当作建模客户端。
...
- 网络延迟是 WOS4 常态。点击 `确定`、`新建`、`保存`、`提交`、`部署`、`启动`、`更新版本`、`进入工程`、`编辑`、`预览链接` 后，先按“点击后观察窗口”判断请求、toast、进度、顶层弹窗和目标文本；观察窗口结束前默认状态是 `loading_pending`，不是失败。
...
- 顶层没有阻塞弹窗，或者弹窗已按按钮文本处理完。
- 相关请求已经发出并返回，或仍处在明确的进度/加载状态。
- 目标业务文本、列表行、iframe、runtime 或按钮状态稳定。
- 如果只是看到旧文本、旧 iframe、旧列表或默认临时名，不能立刻当作最终状态。
