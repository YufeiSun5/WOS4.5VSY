...
-> switch to the data-shape tab: 实时 / 历史 / 计划
-> switch to the object-kind subtab, for example 业务事
-> inspect the object list and details
```

Business events are split by their modeled shape. Do not treat the count under only one shape tab as the complete backend table count:

- `实时 -> 业务事`: real-time business events.
- `历史 -> 业务事`: historical business events.
- `计划 -> 业务事`: planned/scheduled business events.

When expected tables have mixed shapes, check every relevant shape tab and merge the results before declaring a table missing.
...
12. If expected business event objects are missing from one shape tab, switch to the other relevant shape tabs (`实时 / 历史 / 计划`) before calling them absent.
13. Click the runtime custom function copy when method names or runtime copy GUID are needed. Record its `ID / GUID / 母模型GUID / 母模型版本`.
...
If only part of these are visible in one object-management tab, inspect pagination/scroll and switch `实时 / 历史 / 计划` before concluding that the missing tables were not instantiated.

2026-06-27 correction for `PalimpsestL1_0626R2`: the eight Palimpsest business events are split across shape tabs in `时空对象管理平台`. Selecting only `计划 -> 业务事` shows `共 5 条`, which is not the full backend table count.

Verified `计划 -> 业务事` objects:
...
Therefore `pal_assessment_record / pal_assessment_score_detail / pal_operation_log` are not missing; they are historical business events. Backend CRUD functions targeting `pal_assessment_record` should be validated against `历史 -> 业务事`, not `计划 -> 业务事`.
...
- Do not rely only on the total count of the current shape tab. `计划 -> 业务事` and `历史 -> 业务事` can each show a valid subset of the same backend's business events.
- Do not rely only on total card count. Record visible cards, inspect paging or scroll, and check the relevant `实时 / 历史 / 计划` tabs if expected objects are not visible.
